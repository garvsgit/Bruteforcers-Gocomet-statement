...
It is a web scraper scraping google for linkedin profiles;
"""
...
def link_lookup(link):
    try:
        ...
        return row      
    sleep(3) #Maybe necessary to prevent a blockage from Linked-In
...
if __name__ == '__main__':
    ...
    for company in parameters.companies:
        sq=parameters.search_query+' "'+company+'" '
        # driver.get method() will navigate to a page given by the URL address
        ...
        df.to_excel(parameters.file_name+str(iteration)+".xlsx", index=False) #save dataframe as excel file
        iteration+=1
    print('finished')
